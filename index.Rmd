...
ca_geo <- read_csv("index_data/ca_geo_total.csv") %>%
 # mutate(State = "California") %>%
  select( Population, Percentage)

il_geo_sfa <- read_csv("index_data/il_geo_sfa_total.csv")%>%
 # mutate(State = "Illinois SFA")  %>%
  select( Population, Percentage)


il_geo_engie <- read_csv("index_data/il_geo_engie_total.csv")%>%
 # mutate(State = "Illinois Engie")  %>%
  select( Population, Percentage)


nj_geo <- read_csv("index_data/nj_geo_total.csv")%>%
#  mutate(State = "New Jersey")  %>%
  select( Population, Percentage)


ny_geo <- read_csv("index_data/ny_geo_total.csv")%>%
 # mutate(State = "New York")  %>%
  select( Population, Percentage)


ma_geo <- read_csv("index_data/ma_geo_total.csv") %>%
...
ca_lmi <- read_csv("index_data/ca_lmi_total.csv") %>%
  mutate(State = "California") %>%
  select(-Utility)

cal_total <- cbind(ca_geo, ca_lmi)

il_lmi <- read_csv("index_data/il_lmi_total.csv")%>%
...
nj_lmi <- read_csv("index_data/nj_lmi_total.csv")%>%
  mutate(State = "New Jersey") %>%
  select(-Utility)
...
ny_lmi <- read_csv("index_data/ny_lmi_total.csv")%>%
  mutate(State = "New York") %>%
  select(-Utility)
...
nm_lmi <- read_csv("index_data/nm_lmi_total.csv") %>%

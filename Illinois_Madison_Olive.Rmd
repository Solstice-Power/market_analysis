...
townships <- st_read("data/IL/Illinois_Political_Townships/Illinois_Political_Townships.shp") %>%
  filter(NAME %in% c("DORCHESTER TWP", "STAUNTON TWP", "WALSHVILLE TWP", "NEW DOUGLAS TWP", "ALHAMBRA TWP", "OMPHGHENT TWP", "OLIVE TWP"))
...
leaflet() %>%
  ...
  addPolygons(data=add_geo,
             ...
             # stroke=TRUE,
              ...
              dashArray="3",
              fillOpacity=0.55,
              popup=paste("GEOID: ", add_geo$geoid, "<br>",
                          "Total Pop: ", add_geo$total_pop, "<br>",
                          "80% AMI: ", add_geo$AMI_80)) %>%
    ...
    #           fillOpacity=0.55,
    #           popup=paste("GEOID: ", combined_sfa$GEOID)) %>%
  addPolygons(data=townships,
              group="Townships",
              color="red",
              popup=paste("Township Name: ", townships$NAME)) %>%

  
  
  addLayersControl(
    overlayGroups=c( "Geo", "Combined SFA", "Townships"),  # Update the order for consistency
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  hideGroup("Legacy SFA") %>% hideGroup("Current SFA") %>% hideGroup("Combined SFA")
```
...
leaflet() %>%
  ...
  addPolygons(data=add_geo,
    ...
    #           fillOpacity=0.55,
    #           popup=paste("GEOID: ", combined_sfa$GEOID)) %>%
  addPolygons(data=madison_county,
              group="Madison",
              color="red",
              popup=paste("Township Name: ", madison_county$geoid)) %>%

  
  
  addLayersControl(
    overlayGroups=c( "Geo", "Combined SFA", "Madison"),  # Update the order for consistency
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  hideGroup("Legacy SFA") %>% hideGroup("Current SFA") %>% hideGroup("Combined SFA")
```

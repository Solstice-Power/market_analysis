...
ilsfa <- st_read("/Users/jakeford/Library/CloudStorage/GoogleDrive-jake@solstice.us/Shared drives/Product | Jake/Geocoding/ILSFA/Updated Map June 2023/combined_ilsfa_maps.geojson")
...
add_geo <- merge(chas_df, ma_tracts, by="geoid")
...
# st_write(temp, "/Users/jakeford/Library/CloudStorage/GoogleDrive-jake@solstice.us/Shared drives/Product | Jake/Geocoding/MA/MA_2020_EJ_Blocks_corrected.geojson", append=FALSE)
...
temp_util <- st_transform(temp_util, st_crs(add_geo2)) %>%
  st_make_valid()
  ...
  summarize(
          ...
          #  `100% AMI HHs` = sum(AMI_100),
           # `Total Under 80%` = sum(AMI_80_inclusive),
            ...
            `State Total HHs` = sum(total_hh.x),
...
r2_data <- data.frame(
          Utility = c("Eversource", "National Grid", "Unitil", "Total")
)

datatable(temp_table, caption = "Massachusetts Households by AMI Percentage by Utility") %>%
...
## R2 and R4 Meter Estimates

Utility level estimates for the number of R2 and R4 meters in Massachusetts are provided below. The qualification for R2 and R4 is estimated by 200% of the Federal Poverty Line (FPL), we also provided the estimated number of families under 200% FPL within the geoqualified environmental justice (EJ) zones described above. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
r2_data <- read_csv("/Users/jakeford/Library/CloudStorage/GoogleDrive-jake@solstice.us/Shared drives/Product | Jake/Geocoding/Requests/MA_R2_Estimate_Utility_with_EJ.csv") %>%
  select(Utility, `Families Under 200% FPL` = Fam_Under_200, `Families Under 200% FPL in EJ Zone` = Fam_Under_200_EJ) %>%
  filter(Utility != "Municipal") %>%
  mutate(`Total R2 + R4` = case_when(
    Utility == "Eversource" ~ 97911,
    Utility == "National Grid" ~ round(97911/600787*611198), # esimate based on teh R2/R4 combo in Eversource and proportional to 100% AMI 
    Utility == "UNITIL" ~ round(97911/600787*12665)
  ))

test_merge <- merge(temp_table, r2_data, by="Utility") %>%
  adorn_totals("row")

datatable(test_merge, caption = "Massachusetts Households by R2 and R4 Combined Estimates") %>%
  ...
  formatCurrency('State Total HHs',currency = "", interval = 3, digits = 0, mark = ",") %>%
  formatCurrency('State Total Pop',currency = "", interval = 3, digits = 0, mark = ",") %>%
  formatCurrency('Families Under 200% FPL',currency = "", interval = 3, digits = 0, mark = ",") %>%
  formatCurrency('Families Under 200% FPL in EJ Zone',currency = "", interval = 3, digits = 0, mark = ",") %>%
  formatCurrency('Total R2 + R4',currency = "", interval = 3, digits = 0, mark = ",") 
...
There are 145 community solar projects in Massachusetts according to the LIFT solar database. 
